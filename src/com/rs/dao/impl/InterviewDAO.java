...
public class InterviewDAO extends HibernateDaoSupport {
	...
	public Interview findById(java.lang.Integer id) {
		...
		try {
			Interview instance = (Interview) getHibernateTemplate().get(
					"com.rs.model.Interview", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public Interview merge(Interview detachedInstance) {
		...
		try {
			Interview result = (Interview) getHibernateTemplate().merge(
					detachedInstance);
	...
	public static InterviewDAO getFromApplicationContext(ApplicationContext ctx) {
		return (InterviewDAO) ctx.getBean("InterviewDAO");
	}
}